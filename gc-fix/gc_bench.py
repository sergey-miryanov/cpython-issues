...
def main():
    ...
    runner.argparser.add_argument('what', choices=['all', 'array', 'tree'])
    args = runner.parse_args()

    if args.what.lower() in ('all', 'array'):
        arr = make_arr()
        runner.bench_time_func('gc_big_array', bench_gc_collect)
        del arr

    gc.collect(2)

    if args.what.lower() in ('all', 'tree'):
        num_nodes = 3_000_00
        tree = make_tree(num_nodes)
        runner.bench_time_func('gc_big_tree', bench_gc_collect)
        del tree
